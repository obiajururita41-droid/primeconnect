...
export default function WithdrawalPage() {
  ...
  function selectSavedBank(b: SavedBank) {
  ...
  async function verifyAccount() {
    ...
    if (error || data?.error) {
      setError(data?.error || 'Could not verify account. Check the account number and bank.');
      return;
    }
  ...
  async function handleSubmit() {
    ...
    setSuccess(`Transfer initiated successfully. Reference: ${data.reference}`);
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      <p className="text-gray-500 mb-6">
        Available balance: <span className="font-semibold">{currency} {balance.toLocaleString()}</span>
      </p>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {accountName && (
            <p className="text-sm text-green-600 mt-1 font-medium">{accountName}</p>
          )}
        </div>
