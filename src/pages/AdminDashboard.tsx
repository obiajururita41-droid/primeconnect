      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
          {[
            ...
            { label: 'Wallet Balance', value: `₦${(stats?.totalWalletBalance ?? 0).toLocaleString()}`, icon: <Wallet className="w-5 h-5" />, trend: '+8%', sub: 'Total held in wallets', gradient: 'from-blue-600 to-blue-700', glow: 'shadow-blue-500/25', urgent: false },
            { label: 'Total Revenue', value: `₦${(stats?.totalRevenue ?? 0).toLocaleString()}`, icon: <TrendingUp className="w-5 h-5" />, trend: '+15%', sub: 'Lifetime earnings', gradient: 'from-blue-700 to-blue-800', glow: 'shadow-blue-500/25', urgent: false },
            { label: 'Transactions', value: (stats?.totalTransactions ?? 0).toLocaleString(), icon: <ArrowUpRight className="w-5 h-5" />, trend: '+5%', sub: 'All time volume', gradient: 'from-blue-800 to-indigo-800', glow: 'shadow-blue-500/25', urgent: false },
            { label: 'Pending Gift Cards', value: (stats?.pendingGiftCards ?? 0).toLocaleString(), icon: <Gift className="w-5 h-5" />, trend: null, sub: 'Awaiting review', gradient: 'from-indigo-600 to-indigo-700', glow: 'shadow-indigo-500/25', urgent: (stats?.pendingGiftCards ?? 0) > 0 },
            { label: "Today's Activity", value: (stats?.todayTransactions ?? 0).toLocaleString(), icon: <Activity className="w-5 h-5" />, trend: null, sub: 'Transactions today', gradient: 'from-indigo-700 to-blue-900', glow: 'shadow-indigo-500/25', urgent: false },
          ].map((s) => (
