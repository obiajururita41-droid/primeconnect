...
const NETWORKS = [
  ...
  { id: 'etisalat', name: '9mobile', color: 'bg-green-800',  text: 'text-white' },
];
...
export default function AirtimePage() {
  ...
  const handlePurchase = async () => {
    ...
    if (res?.code === '000' || status === 'delivered') {
      ...
      if (debitErr) {
        setError('Airtime sent but wallet debit failed. Contact support: ' + reference);
      } else {
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {NETWORKS.map((n) => (
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-1 ${n.color}`}><span className={`text-xs font-bold ${n.text}`}>{n.name}</span></div>
              ...
              {QUICK_AMOUNTS.map((q) => (
                <button
                  ...
                  className={`py-1.5 rounded-lg text-xs font-medium border transition-all ${
                    amount === String(q) ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-50 text-gray-600 border-gray-200'
                  }`}
                >
                  ₦{q}
                </button>
